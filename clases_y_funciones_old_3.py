...
class regresor_GIN(torch.nn.Module):
    ...
    def __init__(
        ...
        self.GIN_layers =  GIN(in_channels= num_features, hidden_channels= hidden_dim, num_layers= num_layers, out_channels= out_channels, dropout=dropout,  jk='lstm', act='LeakyReLU', act_first = True)#.to('cpu')        
        self.FC1        = Linear(in_features=out_channels, out_features=1, bias=True)#.to('cpu')
        #self.FC2        = Linear(in_features=n_nodes, out_features=1, bias=True)#.to('cpu')        #self.leakyrelu = LeakyReLU(LeakyReLU_slope).to('cpu')
        self.leakyrelu = LeakyReLU(LeakyReLU_slope)#.to('cpu')
    def forward(
        self, x, 
        edge_index, # 
        batch_size
...
#    torch.cpu.init()
#    assert torch.cpu.is_initialized(), "cpu no iniciado!"
