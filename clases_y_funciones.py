...
def cargar_metabolismo():
  ...
  new_reaction(toy, 'G2', 'Producción de ATP y NADH en glicólisis',                  'Glicólisis', "B[c] --> 2 ATP[c] + 2 C0[c] + 2 NADH[c]")
  #Amino Acid
  new_reaction(toy, 'ADH','Síntesis de etanol',                                        'Fermentacion', ".5 C0[c] + 2 NADH[c] <--> 20 Etoh[c]")
  ...
  #TCA cycle
  new_reaction(toy, 'DH1', 'Primera deshidrogenasa del ciclo', 'Ciclo de Krebs', "C0[c] --> 0.7 C1[c] + NADH[c]")
  new_reaction(toy, 'DH2', 'Segunda deshidrogenasa del ciclo', 'Ciclo de Krebs', "C1[c] --> 0.3 C2[c] + NADH[c]")
  new_reaction(toy, 'DH3', 'Tercera deshidrogenasa del ciclo', 'Ciclo de Krebs', "C2[c] --> C0[c]     + NADH[c]")
  #Biomass
  new_reaction(toy, 'GrowthRate', 'Velocidad específica (tasa) de crecimiento', "Producción de biomasa" ,"17 ATP[c] + 7 C0[c] + 2 C1[c] + 3 C2[c] -->  Biomasa")
...
def graficar_experimento(tiempo, concentraciones, flujos):
  ...
  graficar(tiempo, collapsed_Biomass, collapsed_A_diffusion, collapsed_O2_difussion, collapsed_L_diffusion, collapsed_OXPHOS,
           ['Tasa de crecimiento (GrowthRate)',
            ...
            'Produccion de etanol (ETOH_production)', 'Fosforilacion oxidativa (OxPhos)' ])
