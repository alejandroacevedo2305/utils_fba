...
def make_loader(graphs: list, batch_size: int  =250, num_samples: int = 1000):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 1 == 0:
            ...
            if total_eval_loss     < min_total_loss_val:
                ...
                if verbose:
                    print(f"NEW best min_total_loss_val {min_total_loss_val}")
                if save_state_dict:
...
def cargar_metabolismo():
  ...
  # Respiration     
  new_reaction(toy, 'Consumo_de_oxigeno', '', 'Respiracion',                 "O2[e] <--",                       -10, 0)
  new_reaction(toy, 'Oxigeno_entrando_al_citoplasma'   , '', 'Respiracion',                 "O2[e] --> O2[c]",                0, 10)
  new_reaction(toy, 'Fosforilacion_oxidativa'      , '', 'Respiracion',                 "20 NADH[c] + O2[c] --> ATP[c]", 0, 10)
  #new_reaction(toy, 'ATP_sink',     '', 'Respiration',                 "ATP[c] --> ",                   .2, .2)
  #glicolisis
  new_reaction(toy, 'Consumo_de_glucosa', '', 'Glicolisis', "Glc[e] <--")
  new_reaction(toy, 'Glucosa_entrando_al_citoplasma', '', 'Glicolisis', "Glc[e] --> Glc[c]", 0, 10)
  new_reaction(toy, 'R1', '', 'Glicolisis', "ATP[c] + Glc[c] --> B[c]")
  new_reaction(toy, 'R2', '', 'Glicolisis', "B[c] --> 2 ATP[c] + 2 C0[c] + 2 NADH[c]")
  #Amino Acid
  new_reaction(toy, 'R3',          '', 'Fermentacion', ".5 C0[c] + 2 NADH[c] <--> 20 Etoh[c]")
  new_reaction(toy, 'Intercambio_etanol_citoplasma',  '', 'Fermentacion', "Etoh[e] <--> Etoh[c]", -10, 10)
  new_reaction(toy, 'Produccion_de_etanol', '', 'Fermentacion', "Etoh[e] <-->", -10, 10)
  #TCA cycle
  new_reaction(toy, 'DH1', '', 'Ciclo de Krebs', "C0[c] --> 0.7 C1[c] + NADH[c]")
  new_reaction(toy, 'DH2', '', 'Ciclo de Krebs', "C1[c] --> 0.3 C2[c] + NADH[c]")
  new_reaction(toy, 'DH3', '', 'Ciclo de Krebs', "C2[c] --> C0[c]     + NADH[c]")
  #Biomass
  new_reaction(toy, 'Tasa_de_crecimiento', '', 'Crecimiento celular', "17 ATP[c] + 7 C0[c] + 2 C1[c] + 3 C2[c] -->  Biomasa")
  new_reaction(toy, 'Balance_biomasa', '', 'Crecimiento celular', "Biomasa -->")
  toy.objective = 'Tasa_de_crecimiento'
...
def cultivar(model, init_biomasa = .07, init_glucosa = 1, init_oxigeno = .02, init_etanol = 0, horas= 4, init_ATP = .1):
  ...
  def add_dynamic_bounds(model, y):
      ...
      model.reactions.Consumo_de_glucosa.lower_bound = -10 * glucose / (.4 + glucose)

      model.reactions.Consumo_de_oxigeno.lower_bound = -3 * O2 / (2 + O2)
      

      model.reactions.Produccion_de_etanol.lower_bound = -1 * L / (5 + L)
      
      model.reactions.Tasa_de_crecimiento.upper_bound = 1 * biomass / (1 + biomass)
  ...
  def dynamic_system(t, y):
      ...
      with model:
          ...
          lex_constraints = cobra.util.add_lexicographic_constraints(
              model, ['Tasa_de_crecimiento', 'Consumo_de_glucosa', 'Consumo_de_oxigeno', 'Produccion_de_etanol','Fosforilacion_oxidativa', 'R1'], ['max', 'max', 'max', 'max', 'max', 'max'])
...
def graficar(time, A, B, C, D, E, labels ):
  ...
  p1, = ax.plot(time, A, "g-", label=labels[0])
  p2, = twin1.plot(time, B, "r-", label=labels[1])
  p3, = twin2.plot(time, C, "b-", label=labels[2])
  p4, = twin3.plot(time, D, "c-", label=labels[3])
  p5, = twin4.plot(time, E, "y-", label=labels[4])
  #ax.set_xlim(0, .07)
  
 
  ax.set_ylim(   0.9*min(A), max(A))
  
  twin4.set_ylim(0.9*min(E), 1.1*max(E))
  ...
  if max(B) < 0:
      twin1.set_ylim(1.2*min(B), .8*max(B))
  else:
      twin1.set_ylim(0.9*min(B), 1.1*max(B))
      
  if max(C) < 0:
      twin2.set_ylim(   1.1*min(C), .9*max(C))
  else:
...
def graficar_experimento(tiempo, concentraciones, flujos):
  ...
  graficar(tiempo, biomass, glucose, O2, L, ATP, ['Biomasa', 'Concentracion de Glucosa (Glc[e])', 'Concentracion de Oxigeno (O2[e])', 'Concentracion de Etanol (Etoh[e])', 'Concentracion de ATP[c]' ])
  graficar(tiempo, collapsed_Biomass, collapsed_A_diffusion, collapsed_O2_difussion, collapsed_L_diffusion, collapsed_OXPHOS, ['Tasa de crecimiento',
                                                                                                                               'Consumo de glucosa', 'Consumo de oxigeno', 
                                                                                                                               'Produccion de etanol', 'Fosforilacion oxidativa' ])
...
def guardar_experimento(model,tiempo, concentraciones, flujos):
    ...
    new_features = {
    'Tasa_de_crecimiento': np.around(collapsed_Biomass, 7).tolist(),
    'Consumo_de_glucosa': np.around(collapsed_A_diffusion, 7).tolist(),
    'Consumo_de_oxigeno': np.around(collapsed_O2_difussion, 7).tolist(),
    'Produccion_de_etanol': np.around(collapsed_L_diffusion, 7).tolist(),
   'Fosforilacion_oxidativa': np.around(collapsed_OXPHOS, 7).tolist(),
